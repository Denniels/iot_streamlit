...
float readNTCTemperatureCorrected(int pin) {
  /*
  CÁLCULO CORREGIDO para configuración: 5V--NTC--A0--R(10kΩ)--GND
  En esta configuración el voltaje aumenta cuando el NTC se enfría
  */
  ...
  // Resistencia del NTC (FÓRMULA CORREGIDA)
  // V_pin = 5V * R_series / (R_ntc + R_series)
  // Despejando: R_ntc = R_series * (5V - V_pin) / V_pin
  if (voltage >= 5.0) return -999.0;
  
  float ntcResistance = SERIES_RESISTOR * (5.0 - voltage) / voltage;
...
void sendInitMessage() {
  ...
  doc["device_id"] = "arduino_usb_001";
  doc["device_type"] = "arduino_usb";
  doc["firmware_version"] = "1.1_corrected";
  doc["sensors"] = "3x_NTC_10k + 1x_LDR";
  doc["status"] = "ready";
  doc["correction"] = "NTC_formula_fixed";
...
// CORRECCIÓN APLICADA: Fórmula R_ntc = R_series * (5V - V_pin) / V_pin
