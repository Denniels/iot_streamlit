...
class ResilientCloudflareManager:
    ...
    def _signal_handler(self, signum, frame):
        ...
        logger.info("Señal recibida, terminando tunel")
        self.running = False
        self._stop_tunnel()
        ...
        for attempt in range(2):  # Máximo 2 intentos
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"API respuesta {response.status_code} al notificar URL")
                ...
                logger.warning(f"Intento {attempt+1} - No se pudo notificar API: {e}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == 'healthy':
                    ...
                    logger.warning(f"Backend health status: {data.get('status', 'unknown')}")
                    self.stats['backend_failures'] += 1
                    return False
            else:
                logger.warning(f"Backend health check falló: {response.status_code}")
                self.stats['backend_failures'] += 1
                return False
        except requests.exceptions.Timeout:
            logger.warning(f"Backend health check timeout después de {BACKEND_TIMEOUT}s")
            self.stats['backend_failures'] += 1
            return False
        except Exception as e:
    ...
    def _check_tunnel_health(self) -> bool:
        ...
        if not self.current_url:
            logger.debug("No hay URL de túnel para verificar")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == 'healthy':
                    ...
                    logger.warning(f"Túnel health status: {data.get('status', 'unknown')}")
                    self.stats['tunnel_failures'] += 1
                    return False
            else:
                logger.warning(f"Túnel health check falló: {response.status_code}")
                self.stats['tunnel_failures'] += 1
                return False
                
        except requests.exceptions.Timeout:
            logger.warning(f"Túnel health check timeout después de {TUNNEL_TIMEOUT}s")
            self.stats['tunnel_failures'] += 1
            return False
        except Exception as e:
            logger.warning(f"Túnel no accesible: {e}")
            self.stats['tunnel_failures'] += 1
            return False
        ...
        if self.process:
            try:
                ...
                try:
                    ...
                    logger.warning("Forzando terminación del túnel")
                    self.process.kill()
                    self.process.wait()
    ...
    def _restart_tunnel(self) -> bool:
        ...
        if self.last_restart:
            ...
            if time_since_last < 60:  # Menos de 1 minuto desde último restart
                logger.warning(f"Restart muy frecuente, esperando más tiempo...")
            ...
            try:
                ...
                if not process_alive:
                    ...
                    if self.consecutive_failures >= CONSECUTIVE_FAILURES_THRESHOLD:
                        ...
                        logger.warning(f"Túnel unhealthy ({self.consecutive_failures}/{CONSECUTIVE_FAILURES_THRESHOLD})")
                else:
                ...
                if needs_restart:
                    logger.warning(f"Reinicio necesario: {reason}")
    ...
    def run(self):
        ...
        if not self._check_backend_health():
            logger.warning("Backend no disponible, pero iniciando túnel de todas formas")
